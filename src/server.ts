...
// Simple bearer-token auth so randos on the internet can't call your Manus
// account through your server. Set MCP_SERVER_TOKEN in Railway variables and
// put the same value in Claude's connector config.
const MCP_SERVER_TOKEN = process.env.MCP_SERVER_TOKEN;

function isAuthorized(req: express.Request): boolean {
  if (!MCP_SERVER_TOKEN) return true; // no token configured -> open (not recommended)
  const header = req.headers["authorization"];
  if (!header || Array.isArray(header)) return false;
  const [scheme, token] = header.split(" ");
  return scheme === "Bearer" && token === MCP_SERVER_TOKEN;
}
